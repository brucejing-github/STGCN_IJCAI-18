...
from data_loader.my_data_utils import gen_batch
from models.my_tester import model_inference
from models.my_base_model import build_model, model_save
...
def model_train(inputs, indicators, blocks, args, sum_path='./output/tensorboard'):
    '''
    ...
    idx = tf.placeholder(tf.float32, [None, n_his + 1, n, 1], name='data_idx')
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')

    # Define model loss
    train_loss, pred = build_model(x, idx, n_his, Ks, Kt, blocks, keep_prob)
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch):
            ...
            for j, x_idx_batch in enumerate(
                    gen_batch(inputs.get_data('train'), indicators.get_data('train'), batch_size, dynamic_batch=True, shuffle=True)):
                x_batch, idx_batch = x_idx_batch
                summary, _ = sess.run([merged, train_op], feed_dict={x: x_batch[:, 0:n_his + 1, :, :],
                                                                     idx: idx_batch[:, 0:n_his + 1, :, :],
                                                                     keep_prob: 1.0})
            ...
            start_time = time.time()
            min_va_val, min_val = \
                model_inference(sess, pred, inputs, indicators, batch_size, n_his, n_pred, step_idx, min_va_val, min_val)
