...
from utils.my_math_utils import evaluation
...
def model_inference(sess, pred, inputs, indicators, batch_size, n_his, n_pred, step_idx, min_va_val, min_val):
    '''
    ...
    idx_val, idx_test, idx_stats = indicators.get_data('val'), indicators.get_data('test'), indicators.get_stats()
    ...
    evl_val = evaluation(x_val[0:len_val, step_idx + n_his, :, :], y_val, idx_val[0:len_val, step_idx + n_his, :, :], x_stats)
    ...
    if sum(chks):
        ...
        evl_pred = evaluation(x_test[0:len_pred, step_idx + n_his, :, :], y_pred, idx_test[0:len_pred, step_idx + n_his, :, :], x_stats)
        min_val = evl_pred
    return min_va_val, min_val


def model_test(inputs, indicators, batch_size, n_his, n_pred, inf_mode, load_path='./output/models/'):
    '''
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        idx_test = indicators.get_data('test')
        ...
        evl = evaluation(x_test[0:len_test, step_idx + n_his, :, :], y_test,
                         idx_test[0:len_test, step_idx + n_his, :, :], x_stats)
